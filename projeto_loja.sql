...
*/
	BEGIN
		...
		DECLARE @ID_FATURAMENTOI INT
		DECLARE @VALOR_FATURADOI MONEY

		SELECT @ID_ITENSI = ID FROM inserted
		SELECT @VALOR_FATURADOI = (SELECT  [dbo].[FUNC_LUCRO](1))
		SELECT @ID_ITENSI = (SELECT ID FROM ITENS_COMPRA WHERE ID = @ID_ITENSI)
	END

        BEGIN
            UPDATE FATURAMENTO
                SET ID_ITENS_COMPRA = @ID_ITENSI
                    WHERE ID = @ID_FATURAMENTOI

            UPDATE FATURAMENTO
                SET VALOR_LUCRO = @VALOR_FATURADOI
                    WHERE ID = @ID_FATURAMENTOI
        END
GO
